...
def StartQuiz():
    ...
    for QuestionNumber in range(
        ...
        AskedQuestions.append(QuestionData["Question"])
        print(f"\nQuestion:\n{QuestionData['Question']}")
        print("\nOptions:")
        for Key, Value in (QuestionData["Options"].items()):
            print(f"{Key}) {Value}")
        UserAnswer = input(
        ...
        Attempts.append(QuestionNumber)
        Scores.append(CurrentPercentage)
    ...
    if len(WeakTopics) > 0:
        ...
        for WeakTopic in set(
            ...
            try:
                ...
                print("Explanation Service Busy Right Now")
